...
let primitive (p : Clambda_primitives.primitive) (args, approxs)
  ...
  match p with
  | Pmakeblock(tag_int, Asttypes.Immutable, shape, tagdesc) ->
    ...
    Prim (Pmakeblock(tag_int, Asttypes.Immutable, Some shape, tagdesc), args, dbg),
    A.value_block tag (Array.of_list approxs), C.Benefit.zero
  | Praise _ ->
    expr, A.value_bottom, C.Benefit.zero
  | Pmakearray(_, _, _) when is_empty approxs ->
    Prim (Pmakeblock(0, Asttypes.Immutable, Some [], Taglib.default), [], dbg),
    A.value_block (Tag.create_exn 0) [||], C.Benefit.zero
  | Pmakearray (Pfloatarray, Mutable, _tagdesc) ->
      let approx =
        A.value_mutable_float_array ~size:(List.length args)
      in
      expr, approx, C.Benefit.zero
  | Pmakearray (Pfloatarray, Immutable, _tagdesc) ->
      let approx =
