...
let rec expr_size env = function
  ...
  | Uprim(Pmakearray((Paddrarray | Pintarray), _, _tagdesc), args, _) ->
      RHS_block (List.length args)
  | Uprim(Pmakearray(Pfloatarray, _, _tagdesc), args, _) ->
      RHS_floatblock (List.length args)
  | Uprim(Pmakearray(Pgenarray, _, _tagdesc), _, _) ->
...
let rec transl env e =
  ...
  | Uprim(prim, args, dbg) ->
      ...
      | (Pduparray (kind, _), [Uprim (Pmakearray (kind', _, desc), args, _dbg)]) ->
          (* We arrive here in two cases:
             ...
             [Pfloatarray]s. *)
          assert (kind = kind');
          transl_make_array ~desc dbg env kind args
      | (Pduparray _, [arg]) ->
          let prim_obj_dup =
      ...
      | (Pmakearray (kind, _, desc), args) -> transl_make_array ~desc dbg env kind args
...
and transl_make_array ?desc dbg env kind args =
  match kind with
  | Pgenarray ->
...
and transl_prim_2 env p arg1 arg2 dbg =
  ...
  | Pmakeblock (_, _, _, _) | Pfield _ | Psetfield_computed (_, _) | Pfloatfield _
...
and transl_prim_3 env p arg1 arg2 arg3 dbg =
  ...
  | Pbswap16 | Pint_as_pointer | Popaque | Pread_symbol _ | Pmakeblock (_, _, _, _)
